	...
	private async updateTokenUsage(): Promise<void> {
		...
		try {
			...
			if (usage.cachedTokens > 0) {
				tokenText.textContent = `Tokens: ~${usage.estimatedTokens.toLocaleString()} (${uncached.toLocaleString()} new) / ${(100000).toLocaleString()} (${usage.percentUsed}%)`;
			} else {
				tokenText.textContent = `Tokens: ~${usage.estimatedTokens.toLocaleString()} / ${(100000).toLocaleString()} (${usage.percentUsed}%)`;
			}
